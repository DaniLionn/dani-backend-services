...
async function startApp() {
 try {
    await rbx.setCookie("_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_" + cookie);
...
app.get("/riitag", (req, res) => {
    ...
    axios.get(`https://tag.rc24.xyz/wii?game=${id}&key=${process.env.RIITAG_KEY}`).then(function(response_data) {
        ...
        let output = `${err.code} ${err.response.status} (${JSON.stringify(err.response.data)}`
        console.log(output)
...
app.post("/ProxyGet", function(Request, Res) {
    ...
    axios.get(json.url).then((result) => {
...
app.post("/ProxyPost", function(Request, Res) {
    ...
    axios.post(json.url, json.data).then((result) => {
        console.log(result.data)
        Res.send(result.data)
        console.log("returned data to roblox")
    }).catch((err) => {
...
app.get("/robloxUserThumbnail", function(req, res) {
    ...
    axios.get(`https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=${userId}&size=720x720&format=Png&isCircular=true`).then((result) => {
        console.log(result.data)
        res.send(result.data)
        console.log("returned data to roblox")
    }).catch((err) => {
